...
// 1. Configuration Setup (Private to this module)
const dbConfig = {
  leafCollectionDb: process.env.LEAF_COLLECTION_DATABASE || 'BoughtLeaf_Kandedola',
  leafCollectionTable: process.env.LEAF_COLLECTION_TABLE || 'Tr_LeafCollection_Temp',
  leafCountDb: process.env.LEAF_COUNT_DATABASE || 'BoughtLeaf_Kandedola',
  leafCountTable: process.env.LEAF_COUNT_TABLE || 'Reg_LeafCount',
  schema: process.env.DB_SCHEMA || 'dbo'
};

// Helper to get full table names
const getTableName = (db, table) => `[${db}].[${dbConfig.schema}].[${table}]`;

const leafCollectionFullName = getTableName(dbConfig.leafCollectionDb, dbConfig.leafCollectionTable);
const leafCountFullName = getTableName(dbConfig.leafCountDb, dbConfig.leafCountTable);

// Immediate log to confirm the service is loaded
console.log('📊 LeafCountService Module loaded with:', {
  leafCollectionTable: leafCollectionFullName,
  leafCountTable: leafCountFullName
});

/**
 * Get all distinct routes
 */
const getDistinctRoutes = async () => {
  try {
    const pool = await getMainConnection();
    console.log('📊 Fetching routes from:', leafCollectionFullName);
    
    const result = await pool.request().query(`
      SELECT DISTINCT [Route]
      FROM ${leafCollectionFullName}
      WHERE [Route] IS NOT NULL 
        AND [Route] != ''
        AND [Route] != 'null'
        AND LTRIM(RTRIM([Route])) != ''
    ...
    return result.recordset.map(item => item.Route?.trim()).filter(Boolean);
  } catch (error) {
    console.error('❌ Error in getDistinctRoutes:', error);
    throw error;
  }
 ...
 */
const getRouteTotalWeight = async (routeName, date, month) => {
  try {
    const pool = await getMainConnection();
    ...
    const monthNumber = monthMap[monthAbbr];
    if (!monthNumber || !year) throw new Error('Invalid month format');
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT 
          ISNULL(SUM([Gross]), 0) as TotalGross,
          (ISNULL(SUM([Coarse]), 0) + ISNULL(SUM([Water]), 0) + ISNULL(SUM([BagWeight]), 0) + 
           ISNULL(SUM([Spd]), 0) + ISNULL(SUM([Boild]), 0) + ISNULL(SUM([Rejected]), 0) +
           ISNULL(SUM([RouteDeduct]), 0) + ISNULL(SUM([Excess_Leaf]), 0) + 
           ISNULL(SUM([Transfer]), 0) + ISNULL(SUM([RouteDeductPre]), 0)) as TotalDeductions
        FROM ${leafCollectionFullName}
        WHERE LTRIM(RTRIM([Route])) = LTRIM(RTRIM(@route))
          AND CAST([LogTime] AS DATE) = @targetDate
      `);

    const { TotalGross, TotalDeductions } = result.recordset[0];
    const netWeight = TotalGross - TotalDeductions;
...
/**
 * Save leaf count to database
 */
const saveLeafCount = async (data) => {
  try {
    const pool = await getMainConnection();

    const result = await pool.request()
      .input('date', sql.Int, parseInt(data.date))
      .input('month', sql.NVarChar, data.month)
      .input('route', sql.NVarChar, data.route.trim())
      .input('bestLeaf', sql.Int, data.bestLeaf || 0)
      .input('belowBest', sql.Int, data.bellowBest || 0)
      .input('poor', sql.Int, data.poor || 0)
      .input('user', sql.NVarChar, data.userName || 'mobile_user')
      .input('pcName', sql.NVarChar, os.hostname())
      .query(`
        INSERT INTO ${leafCountFullName} ([Date], [Month], [Route], [BestLeaf], [BelowBest], [Poor], [User], [PC_Name], [LogTime])
        VALUES (@date, @month, @route, @bestLeaf, @belowBest, @poor, @user, @pcName, GETDATE());
        SELECT SCOPE_IDENTITY() as Ind;
      `);

    return { success: true, ind: result.recordset[0].Ind };
  } catch (error) {
    console.error('❌ Error in saveLeafCount:', error);
    throw error;
  }
...
/**
 * Get leaf count history with optional filters
 */
const getLeafCountHistory = async (filters = {}) => {
  try {
    const pool = await getMainConnection();
